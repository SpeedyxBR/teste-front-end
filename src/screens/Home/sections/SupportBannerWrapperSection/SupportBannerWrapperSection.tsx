...
export const SupportBannerWrapperSection = (): JSX.Element => {
  ...
  return (
    <section className="w-full py-8">
      ...
      {/* Grid banners no desktop */}
      <div className="hidden md:grid grid-cols-2 gap-8 max-w-[1280px] mx-auto">
        {banners.map((banner, index) => (
          <Card
            key={`banner-grid-${index}`}
            className="relative w-full h-[350px] overflow-hidden rounded-[20px] border-0"
          >
            <div
              className="absolute inset-0 bg-cover bg-center"
